...
export default function AccountingPage() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-2xl font-bold">Accounting</h1><p className="mt-1 text-sm text-muted-foreground">Manage your chart of accounts, journals, and financial statements.</p></div><div className="flex gap-2"><Button variant="outline" onClick={load}><RefreshCw className="mr-2 h-4 w-4" />Refresh</Button><Button onClick={() => setEntryOpen(true)}><FilePlus2 className="mr-2 h-4 w-4" />Journal entry</Button></div></div>
    ...
    <Dialog open={accountOpen} onOpenChange={setAccountOpen}><DialogContent><form onSubmit={createAccount}><DialogHeader><DialogTitle>Add account</DialogTitle></DialogHeader><div className="space-y-3 py-4"><Label>Account code<Input required value={accountForm.code} onChange={e => setAccountForm({ ...accountForm, code: e.target.value })} /></Label><Label>Account name<Input required value={accountForm.name} onChange={e => setAccountForm({ ...accountForm, name: e.target.value })} /></Label><Label>Type<select className="mt-1 h-9 w-full rounded-md border bg-background px-3" value={accountForm.type} onChange={e => setAccountForm({ ...accountForm, type: e.target.value })}>{types.map(type => <option key={type}>{type}</option>)}</select></Label></div><DialogFooter><Button type="submit" disabled={saving}>Save account</Button></DialogFooter></form></DialogContent></Dialog>
    <Dialog open={entryOpen} onOpenChange={setEntryOpen}><DialogContent className="sm:max-w-2xl"><form onSubmit={createEntry}><DialogHeader><DialogTitle>New journal entry</DialogTitle></DialogHeader><div className="space-y-3 py-4"><div className="grid grid-cols-2 gap-3"><Label>Date<Input type="date" value={entryDate} onChange={e => setEntryDate(e.target.value)} /></Label><Label>Description<Input value={entryDescription} onChange={e => setEntryDescription(e.target.value)} /></Label></div>{lines.map((line, i) => <div key={i} className="grid grid-cols-[1fr_110px_110px] gap-2"><select required className="h-9 rounded-md border bg-background px-2" value={line.accountId} onChange={e => setLine(i, { accountId: e.target.value })}><option value="">Select account</option>{accounts.filter(a => a.isActive).map(a => <option value={a.id} key={a.id}>{a.code} — {a.name}</option>)}</select><Input type="number" min="0" step="0.01" placeholder="Debit" value={line.debit || ''} onChange={e => setLine(i, { debit: Number(e.target.value), credit: 0 })} /><Input type="number" min="0" step="0.01" placeholder="Credit" value={line.credit || ''} onChange={e => setLine(i, { credit: Number(e.target.value), debit: 0 })} /></div>)}<Button type="button" variant="outline" size="sm" onClick={() => setLines([...lines, { accountId: '', debit: 0, credit: 0 }])}>Add line</Button><p className={Math.abs(debit - credit) < .001 ? 'text-sm text-emerald-600' : 'text-sm text-destructive'}>Debits {money(debit)} · Credits {money(credit)}</p></div><DialogFooter><Button type="submit" disabled={saving || Math.abs(debit - credit) > .001}>Save draft</Button></DialogFooter></form></DialogContent></Dialog>
  </div>;
}

function ReportTable({ title, rows, leftLabel, rightLabel, totalLeft, totalRight }: { title: string; rows: { code: string; name: string; left: number; right: number }[]; leftLabel: string; rightLabel: string; totalLeft: number; totalRight: number }) { return <Card><CardHeader><CardTitle>{title}</CardTitle></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>Code</TableHead><TableHead>Account</TableHead><TableHead>{leftLabel}</TableHead><TableHead>{rightLabel}</TableHead></TableRow></TableHeader><TableBody>{rows.map(row => <TableRow key={row.code}><TableCell>{row.code}</TableCell><TableCell>{row.name}</TableCell><TableCell>{money(row.left)}</TableCell><TableCell>{money(row.right)}</TableCell></TableRow>)}<TableRow className="font-bold"><TableCell colSpan={2}>Total</TableCell><TableCell>{money(totalLeft)}</TableCell><TableCell>{money(totalRight)}</TableCell></TableRow></TableBody></Table></CardContent></Card>; }

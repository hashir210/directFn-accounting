...
export function InvoiceViewerDialog({ invoiceId, open, onOpenChange }: InvoiceViewerDialogProps) {
  ...
  const paymentUrl = invoice ? (typeof window !== 'undefined' ? `${window.location.origin}/pay/${invoice.id}` : `https://example.com/pay/${invoice.id}`) : '';

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto print:max-w-none print:max-h-none print:overflow-visible print:border-none print:shadow-none print:p-0">
        <DialogTitle className="sr-only">View Invoice</DialogTitle>
        <DialogDescription className="sr-only">Detailed view of the invoice including printable PDF format.</DialogDescription>

        {loading && <div className="flex justify-center p-12"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>}
        
        {!loading && !invoice && <div className="p-12 text-center text-muted-foreground">Invoice not found or failed to load.</div>}
        
        {!loading && invoice && (
          <div className="space-y-6 pb-6 print:p-0 print:m-0 print:block">
            {/* Non-printable Action Bar inside Modal */}
            <div className="flex items-center justify-end space-x-3 print:hidden border-b pb-4">
            ...
            <div ref={invoiceRef} className="bg-white text-black p-8 print:p-0">
